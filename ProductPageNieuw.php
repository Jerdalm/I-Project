...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if(isset($_POST['submit-file'])){
        $target_dir = "./uploads/user/" . $productdata[0]['verkoper'] . '/' . $productdata[0]['voorwerpnummer']. '/';
        if (!file_exists($target_dir)){
            mkdir('uploads/user/'. $productdata[0]['verkoper'] . '/' . $productdata[0]['voorwerpnummer'] , 02202, true);
        }
        ...
        if(checkIfImage($_POST["submit-file"]) && checkAllowedFileTypes($imageFileType) && checkSizeFile(500000) && checkExistingFile($target_file)) {
            ...
            if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                ...
                $uploadParameters = array(':voorwerpnummer' => $productdata[0]['voorwerpnummer'] , ':bestandsnaam' => $bestandsnaam);
            ...
            <div class="row">
                <div class="col-lg-7 p-3 bg-secondary text-white">
                    ...
                    <div class="col p-3 mb-2 bg-secondary text-white" style="text-align: left">
                        ...
                        <dl class="dl-horizontal">
                            <dt>Beschrijving:</dt>
                            <br>
                            <dd><?= $productdata[0]['beschrijving'] ?></dd>                            
                        </dl>
                    </div>
                </div>
                ...
                <div class="col-lg-5 p-3 bg-secondary text-white">
                    <div class="alert alert-dark" role="alert">
                        <p class="beginTijdstip"><i>Aangeboden op: <?= $productdata[0]['looptijdBeginDag'] .' om: ' . date_format(date_create($productdata[0]['looptijdBeginTijdstip']), "H:i") ?> </i></p>
                        <h2 class="alert-heading"> <strong> <?= $productdata[0]['titel']?></strong></h2> 
                        <p>Startprijs: € <?=$productdata[0]['startprijs']?></p>
                        <p>Verzendkosten: € <?=$productdata[0]['verzendkosten']?></p>
                        <p>Productnummer: <?=$productdata[0]['voorwerpnummer']?></p>
                        <div id="txt"></div>
                        <hr>
